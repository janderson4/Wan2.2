...
class WanI2V:
    ...
    def generate(self,
        ...
        with (
            ...
            ########################################
            def score_noise_gaussianity(noise_estimate):
                """
                Score how Gaussian the noise estimate looks.
                Higher score = more Gaussian = better.
                Combines multiple metrics.
                """
                # Metric 1: Mean should be close to 0
                mean_score = -torch.abs(noise_estimate.mean()).item()
                
                # Metric 2: Std should be close to 1
                std = noise_estimate.std().item()
                std_score = -abs(std - 1.0)
                
                # Metric 3: Check normality via kurtosis (should be ~3 for Gaussian)
                # Flatten for statistics
                flat = noise_estimate.flatten()
                mean = flat.mean()
                std_val = flat.std()
                if std_val > 1e-6:
                    fourth_moment = ((flat - mean) ** 4).mean()
                    kurtosis = fourth_moment / (std_val ** 4)
                    kurtosis_score = -abs(kurtosis - 3.0)
                else:
                    kurtosis_score = -100.0
                
                # Metric 4: Spatial autocorrelation should be low
                # Check correlation between adjacent spatial locations
                if noise_estimate.ndim >= 3:
                    # Take a 2D slice
                    slice_2d = noise_estimate[0, 0] if noise_estimate.ndim == 4 else noise_estimate[0]
                    if slice_2d.numel() > 1:
                        diff_h = (slice_2d[:-1, :] * slice_2d[1:, :]).mean().item()
                        diff_w = (slice_2d[:, :-1] * slice_2d[:, 1:]).mean().item()
                        autocorr_score = -(abs(diff_h) + abs(diff_w))
                    else:
                        autocorr_score = 0.0
                else:
                    autocorr_score = 0.0
                ...
                total_score = (
                    1.0 * mean_score +      # Mean near 0
                    2.0 * std_score +        # Std near 1
                    1.0 * kurtosis_score +   # Kurtosis near 3
                    0.5 * autocorr_score     # Low spatial correlation
                )
            ...
            def estimate_noise_endpoint(latent, sigma, model, timestep_val, arg_c, arg_null, guide_scale_val):
                ...
                with torch.no_grad():
                    ...
                    v_cond = model(latent_input, t=t_tensor, **arg_c)[0]
                    v_uncond = model(latent_input, t=t_tensor, **arg_null)[0]
                    v_pred = v_uncond + guide_scale_val * (v_cond - v_uncond)
                    
                    # Estimate noise endpoint: x1_hat = x_t + (1-sigma)*v
                    x1_estimate = latent + (1.0 - sigma) * v_pred
            ...
            # Search bounds (sigma/t typically in [0, 1])
            a, b = 0.0, 1.0

            print("\n" + "="*60)
            print("GOLDEN SECTION SEARCH FOR TRUE SIGMA")
            print("="*60)

            # Prepare model and arguments for inference
            ...
            while abs(b - a) > tol and iteration < max_iterations:
                iteration += 1
                print(f"\n--- Iteration {iteration} ---")
                print(f"Search interval: [{a:.6f}, {b:.6f}] (width: {b-a:.6f})")
            ...
            # You can now use optimal_sigma instead of your computed sigma
            sigma = optimal_sigma
            ########################################
